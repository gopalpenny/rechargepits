...
Additionally, initial water content (`VWC_0`), porosity (`n`), saturated hydraulic conductivity (`Ksat`), and the time(s) at which the infiltration should be estimated (`times`). 
...
get_pit_recharge_rectangular(height, length, width, n, Ksat, VWC_0, h_0, times)
```
...
## Cylindrical pit with cylindrical and vertical flow

Here the pit is represented by a radius and a depth. 
