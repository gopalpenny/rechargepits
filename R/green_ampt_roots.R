...
#' @param green_ampt_function_name Name of function to use in root finding
#' @param ... Parameters passed to the function named by `green_ampt_function`
#' @export
#' @examples
#' library(units)
...
get_greenampt_x_roots <- function(times, x_units, green_ampt_function_name, ...) {
  ...
  for (i in 1:length(times)) {
    ...
    x_i <- units::set_units(stats::uniroot(green_ampt_function_i, c(0, 1e10))$root, x_units)
